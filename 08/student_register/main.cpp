...
void perezapis(const std::string& file_name, std::map< std::string, Student* >& numerical_order)
{
    std::ofstream fail_object(file_name);
    for(auto it1 = numerical_order.begin(); it1 != numerical_order.end(); it1++)
    {
        fail_object << it1->second->student_number << ";" << it1->second->user_id << ";" << it1->second->name << ";" << it1->second->phone_number << ";" << it1->second->email << ";" << it1->second->skype << std::endl;
    }
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            if(parts.size() != 2){
                ...
                for(auto it = student_numbers.begin(); it != student_numbers.end(); it++)
                {
                    if(it->second->student_number == parts.at(1))
                    {
